...
class PluginSystem {
    ...
    async handleMessage(serializedMsg, bot) {
        ...
        if (serializedMsg.isStatus) {
            if (this.eventPlugins.has('status')) {
                const plugins = this.eventPlugins.get('status');
                for (const plugin of plugins) {
                    try {
                        ...
                        console.error(`Error executing status event plugin:`, error);
                    }
                }
            }
        ...
        // Process "all" event plugins (they receive all message types except status)
        if (this.eventPlugins.has('all')) {
            const plugins = this.eventPlugins.get('all');
            for (const plugin of plugins) {
                try {
                    ...
                    console.error(`Error executing "all" event plugin:`, error);
                }
            }
        }

        // Process specific event plugins
        ...
        // Process mentions
        if (serializedMsg.mentionedJid && serializedMsg.mentionedJid.length > 0) {
            if (this.eventPlugins.has('mention')) {
                const plugins = this.eventPlugins.get('mention');
                for (const plugin of plugins) {
                    try {
                        // Check if bot is mentioned
                        const botJid = bot.user?.id || '';
                        const isBotMentioned = serializedMsg.mentionedJid.some(jid => 
                            jid.includes(botJid.replace(/[^0-9]/g, ''))
                        );

                        if (!isBotMentioned && plugin.requireBotMention) {
                            continue;
                        }
                        ...
                        console.error(`Error executing mention plugin:`, error);
                    }
                }
            }
        }
