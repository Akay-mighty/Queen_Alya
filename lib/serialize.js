...
async function resolveLidToJid(sock, lidOrJid) {
    ...
    // Normalize the LID input
    let normalizedLid = lidOrJid.toString().trim();
    
    // Handle cases where lid is in format @123456789 (remove @ and add @lid)
    if (normalizedLid.startsWith('@') && !normalizedLid.endsWith('@lid')) {
        normalizedLid = normalizedLid.substring(1) + '@lid';
    }
    // Handle cases where lid is just numbers (add @lid)
    else if (/^\d+$/.test(normalizedLid)) {
        normalizedLid = normalizedLid + '@lid';
    }
    // Ensure it ends with @lid if not already
    else if (!normalizedLid.endsWith('@lid')) {
        normalizedLid = normalizedLid.split('@')[0] + '@lid';
    }
    
    try {
        // First try to find in any active group metadata
        const groupFetchPromises = [];
        const groupJids = [];
        
        // Get all groups the bot is in
        const groups = sock.groupFetchAllParticipating().catch(() => ({}));
        const groupData = await groups;
        
        if (groupData) {
            for (const groupJid in groupData) {
                if (groupData[groupJid].participants) {
                    groupFetchPromises.push(
                        sock.groupMetadata(groupJid).catch(() => null)
                    );
                    groupJids.push(groupJid);
                }
            }
        }
        
        // Wait for all metadata fetches to complete
        const groupMetadatas = await Promise.all(groupFetchPromises);
        
        // Check all group participants for matching LID
        for (let i = 0; i < groupMetadatas.length; i++) {
            const metadata = groupMetadatas[i];
            if (!metadata) continue;
            
            const participant = metadata.participants.find(p => 
                p.lid && normalizeJid(p.lid) === normalizeJid(normalizedLid)
            );
        ...
        // Fallback to onWhatsApp check if not found in groups
        const results = await sock.onWhatsApp(normalizedLid);
        if (results && results.length > 0) {
            return normalizeJid(results[0].jid);
        }
        
        // If still not found, try with the original input (might be a phone number)
        if (normalizedLid !== lidOrJid) {
            const fallbackResults = await sock.onWhatsApp(lidOrJid);
            if (fallbackResults && fallbackResults.length > 0) {
                return normalizeJid(fallbackResults[0].jid);
            }
        }
    ...
    // Return the normalized LID if we couldn't resolve it to a JID
    return normalizeJid(normalizedLid);
}
...
async function smsg(sock, m, str) {
    ...
    if (m.key) {
        ...
        if (m.sender.includes('@lid') || /^@?\d+$/.test(m.sender)) {
            try {
                const resolvedJid = await resolveLidToJid(sock, m.sender);
        ...
        if (m.quoted) {
            ...
            if(quoted.viewOnceMessageV2) {
                ...
                if (m.msg.contextInfo?.participant) {
                    ...
                    try {
                        const resolvedJid = await resolveLidToJid(sock, quotedParticipant);
                        if (resolvedJid) {
