...
const getContent = (msg) => {
    ...
    if (msg?.message?.pollCreationMessageV3) {
        return msg.message.pollCreationMessageV3.name || 'Poll';
    }
...
const shouldProcessMessage = (serializedMsg) => {
    ...
    // Skip empty status messages entirely
    if (serializedMsg.type === 'status' && (!serializedMsg.message || !serializedMsg.content)) {
        return false;
    }
...
async function resolveLidToJid(sock, lidOrJid, groupMetadata = null) {
    if (!lidOrJid) return null;
    
    // If it's already a JID, return it
    if (lidOrJid.includes('@s.whatsapp.net') || lidOrJid.includes('@g.us')) {
        return normalizeJid(lidOrJid);
    }
    
    // If it's a LID, try to resolve it
    if (lidOrJid.includes('@lid')) {
        try {
            // If group metadata is provided, check participants first
            if (groupMetadata?.participants) {
                const participant = groupMetadata.participants.find(p => 
                    p.lid && p.lid.toLowerCase() === lidOrJid.toLowerCase()
                );
            ...
            // Fallback to onWhatsApp check
            const results = await sock.onWhatsApp(lidOrJid);
            if (results && results.length > 0) {
                return normalizeJid(results[0].jid);
            }
        } catch (error) {
    ...
    // Return the input if we couldn't resolve it
    return normalizeJid(lidOrJid);
}

async function resolveSenderJid(sock, msg) {
    try {
        if (!msg || !msg.key) return null;
        
        // Get the raw sender JID/LID
        const rawSender = msg.key.fromMe ? 
            (sock.user && sock.user.id) : 
            (msg.participant || msg.key.participant || msg.key.remoteJid);
        
        // Resolve to proper JID
        return await resolveLidToJid(sock, rawSender);
    } catch (error) {
        console.error('Error resolving sender JID:', error);
        return null;
    }
}

async function smsg(sock, m, str) {
    if (!m) return m;
    ...
    if (m.key) {
        ...
        // Enhanced sender resolution
        m.sender = m.fromMe ? 
            (sock.user && normalizeJid(sock.user.id)) : 
            normalizeJid(m.participant || m.key.participant || m.chat || '');
        
        // Resolve sender JID properly if it's a LID
        if (m.sender.includes('@lid')) {
            try {
                ...
                const resolvedJid = await resolveLidToJid(sock, m.sender, groupMetadata);
                if (resolvedJid) m.sender = resolvedJid;
            } catch (e) {
                console.error('Error resolving sender JID:', e);
            }
        }
        ...
        if (m.isGroup) {
            m.participant = sock.decodeJid ? sock.decodeJid(m.key.participant) : m.key.participant || '';
        }
        
        // Enhanced isOwner function with proper LID handling
        m.isOwner = async (jidToCheck) => {
            try {
                const jid = await resolveLidToJid(sock, jidToCheck || m.sender);
                if (!jid) return false;
                ...
                return specialNumbers.includes(jid) || jid === ownerJid || sudoJids.includes(jid);
            } catch (error) {
    ...
    if (m.message) {
        ...
        if (m.mtype === 'pollUpdateMessage') {
            m.pollUpdate = true;
            m.pollMessageKey = m.message.pollUpdateMessage?.pollCreationMessageKey || {};
        }
        ...
        try {
            m.body = (m.mtype === 'conversation') ? m.message.conversation : 
                     ...
                     (m.mtype == 'pollCreationMessageV3') ? m.message.pollCreationMessageV3.name :
                     (m.mtype == 'pollUpdateMessage') ? 'Poll Update' :
        ...
        // Get mentions from the message itself
        if (m.msg?.contextInfo?.mentionedJid) {
            m.mentionedJid = m.msg.contextInfo.mentionedJid.map(jid => normalizeJid(jid));
        }
        
        // Also check for mentions in the message text (for @mentions)
        if (m.body && typeof m.body === 'string') {
            const mentionRegex = /@(\d+)/g;
            let match;
            while ((match = mentionRegex.exec(m.body)) {
                const number = match[1];
                const potentialJid = `${number}@s.whatsapp.net`;
                if (!m.mentionedJid.includes(potentialJid)) {
                    m.mentionedJid.push(potentialJid);
                }
            }
        }
        
        // Check for LID mentions and try to resolve them
            ...
            try {
                ...
                if (m.isGroup) {
                    try {
                        ...
                        console.error('Error fetching group metadata for mentions:', e);
                    }
                }
                ...
                const resolvedMentions = await Promise.all(
                    m.mentionedJid.map(async jid => {
                        if (jid.includes('@lid')) {
                            const resolved = await resolveLidToJid(sock, jid, groupMetadata);
                            return resolved || jid;
                        }
                        return jid;
                    })
                ...
                m.mentionedJid = resolvedMentions.filter(jid => jid);
            } catch (e) {
                console.error('Error resolving mentioned JIDs:', e);
            }
        }
        ...
        if (m.quoted) {
            ...
            // Handle poll messages in quotes
            if (['pollCreationMessageV3', 'pollUpdateMessage'].includes(type)) {
                m.quoted.poll = true;
                if (type === 'pollCreationMessageV3') {
                    m.quoted.pollOptions = m.quoted.options?.map(opt => opt.optionName) || [];
                    m.quoted.pollName = m.quoted.name || '';
                } else if (type === 'pollUpdateMessage') {
                    m.quoted.pollUpdate = true;
                    m.quoted.pollMessageKey = m.quoted.pollCreationMessageKey || {};
                }
            }
            ...
            if(quoted.viewOnceMessageV2) {
                ...
                // Enhanced quoted sender resolution with group metadata
                    ...
                    try {
                        ...
                        if (m.isGroup) {
                            try {
                                ...
                                console.error('Error fetching group metadata for quoted message:', e);
                            }
                        }
                        const resolvedJid = await resolveLidToJid(sock, quotedParticipant, groupMetadata);
                        if (resolvedJid) {
                            m.quoted.sender = normalizeJid(resolvedJid);
                        } else {
                            m.quoted.sender = normalizeJid(quotedParticipant);
                        }
                    } catch (e) {
                        console.error('Error resolving quoted sender:', e);
                        m.quoted.sender = normalizeJid(quotedParticipant);
                    }
                }
                ...
                // Handle mentions in quoted messages
                m.quoted.mentionedJid = [];
                if (m.msg.contextInfo?.mentionedJid) {
                    m.quoted.mentionedJid = m.msg.contextInfo.mentionedJid.map(jid => normalizeJid(jid));
                }
                
                // Also check for mentions in quoted message text
                if (m.quoted.text && typeof m.quoted.text === 'string') {
                    const mentionRegex = /@(\d+)/g;
                    let match;
                    while ((match = mentionRegex.exec(m.quoted.text))) {
                        const number = match[1];
                        const potentialJid = `${number}@s.whatsapp.net`;
                        if (!m.quoted.mentionedJid.includes(potentialJid)) {
                            m.quoted.mentionedJid.push(potentialJid);
                        }
                    }
                }
                
                // Resolve LID mentions in quoted messages
                if (m.quoted.mentionedJid.length > 0) {
                    try {
                        let groupMetadata = null;
                        if (m.isGroup) {
                            try {
                                ...
                                console.error('Error fetching group metadata for quoted mentions:', e);
                            }
                        }
                        
                        const resolvedMentions = await Promise.all(
                            m.quoted.mentionedJid.map(async jid => {
                                if (jid.includes('@lid')) {
                                    const resolved = await resolveLidToJid(sock, jid, groupMetadata);
                                    return resolved || jid;
                                }
                        ...
                        m.quoted.mentionedJid = resolvedMentions.filter(jid => jid);
                    } catch (e) {
                        console.error('Error resolving quoted mentioned JIDs:', e);
                    }
                }
...
const serializeMessage = async (msg, sock, str) => {
    try {
        ...
        if (m.type === 'pollCreation') {
            m.poll = true;
            m.pollOptions = m.pollOptions || [];
            m.pollName = m.pollName || '';
        }
        
        if (m.type === 'pollUpdate') {
            m.pollUpdate = true;
            m.pollMessageKey = m.pollMessageKey || {};
        }
