...
const getMessageType = (msg) => {
    ...
    if (chatType === 'status') {
        if (msg?.message?.extendedTextMessage) return 'statusText';
        if (msg?.message?.imageMessage) return 'statusImage';
        if (msg?.message?.videoMessage) return 'statusVideo';
        return 'status';
    }
...
const getContent = (msg) => {
    ...
    if (msg?.message?.statusMessage) {
        return msg.message.statusMessage.text || '';
    }
    
    // Handle status updates (image/video/text)
    if (getChatType(msg?.key?.remoteJid) === 'status') {
...
const shouldProcessMessage = (serializedMsg) => {
    ...
    // Process all status messages, even empty ones
        ...
        try {
            ...
            if (groupMetadata?.participants) {
                const participant = groupMetadata.participants.find(p => 
                    ...
                    (p.id && p.id.toLowerCase() === lidOrJid.toLowerCase())
                );
    ...
    if (m.message) {
        ...
        try {
            m.body = (m.mtype === 'conversation') ? m.message.conversation : 
                     ...
                     (m.mtype == 'statusMessage') ? m.message.statusMessage.text || '' :
        ...
        if (m.quoted) {
            ...
            if(quoted.viewOnceMessageV2) {
                ...
                m.quoted.isBot = m.quoted.id ? m.quoted.id.startsWith('3EB0') : false;
                m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('3EB0') : false;
